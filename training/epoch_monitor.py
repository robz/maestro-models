...
class EpochMonitor:
  ...
  def restore(self, model, model_name):
    filename = F'{self.directory}/{model_name}'
  ...
  def epoch_cb(self, epoch, train_loss, train_acc, val_loss, val_acc, time_elapsed, model):
    ...
    self.save(state, F'{self.directory}/{self.name}-recent.pt')
    if val_loss < self.best_val_loss:
      self.best_val_loss = val_loss
      self.save(state, F'{self.directory}/{self.name}-best.pt')
    if epoch % self.model_save_period == 0:
      self.save(state,  F'{self.directory}/{self.name}-epoch{epoch}.pt')
